# Lembar Penilaian Juri — LKS SMK TKJ — Linux Server Administration (Debian 13)
...
> **Sistem Penilaian**
>
> - Teknikal (konfigurasi berjalan): **58 poin** + 2 poin bonus NAT (opsional)
...
## BAGIAN A — PENILAIAN TEKNIKAL (58 poin + 2 bonus NAT)
...
| 6 | Client mendapat IP dari DHCP (verifikasi lease atau `ip addr` di PC client) | 2 | | |
...
| 10 | `http://NAMASEKOLAH.lan` dapat diakses dari PC client | 2 | | |
...
| 5 | `mail_driver = maildir` dan `mail_path = ~/Maildir` dikonfigurasi di Dovecot | 1 | | |
...
| 9 | Email lokal dapat dikirim dan diterima (cek via `mail -f ~/Maildir/` atau log) | 1 | | |

**Subtotal A6**: ___ / 10

---

**TOTAL PENILAIAN TEKNIKAL (A1–A6)**: ___ / 58
...
| 2 | **Jelaskan fungsi nilai Refresh, Retry, dan Expire di SOA.** *(Jawaban: Refresh = seberapa sering secondary DNS cek ke primary; Retry = jika refresh gagal, tunggu berapa lama sebelum coba lagi; Expire = jika primary terus tidak bisa dicapai, secondary masih melayani data sampai waktu ini habis.)* | 3 | | |
...
| 2 | **Mengapa permission direktori home harus 711, bukan 755 atau 700?** *(Jawaban: 711 = owner rwx, group x, other x. Apache perlu bisa masuk (execute) ke direktori home untuk membaca public_html. 755 juga benar tapi kurang aman. 700 akan block Apache.)* | 3 | | |
...
| # | Pertanyaan | Poin | Nilai | Catatan |
| --- | --- | --- | --- | --- |
| 1 | **Apa perbedaan Postfix dan Dovecot dalam sistem email?** *(Jawaban: Postfix = MTA (Mail Transfer Agent), bertugas mengirim dan menerima email antar server via SMTP port 25. Dovecot = MDA (Mail Delivery Agent), bertugas menyimpan email dan melayani client via IMAP port 143 / POP3 port 110.)* | 3 | | |
...
| 3 | **Demonstrasikan cara mengirim email dari terminal dan cara membaca inbox.** *(Jawaban: `echo "pesan" \| mail -s "subjek" user@domain.lan` untuk kirim; `mail -f ~/Maildir/` untuk buka inbox; nomor pesan untuk baca; `q` untuk keluar.)* | 2 | | |
| 4 | **Di mana Anda bisa melihat log untuk troubleshoot masalah pengiriman email?** *(Jawaban: `journalctl -u postfix -n 30` — berisi semua aktivitas Postfix. Cari kata `status=sent`, `status=bounced`, atau error message.)* | 2 | | |
